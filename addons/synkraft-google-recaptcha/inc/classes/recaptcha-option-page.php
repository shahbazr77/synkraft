...
class Recaptcha_Option_Page {
    ...
    public function show_on_woo_checkout_callback_disabeled() {
        $show_on_checkout = get_option( 'enable_captcha_on_woo_checkout' );
        echo '<input type="checkbox" class="form-check-input" '.$this->status_license.' name="show_on_checkout" id="woo_checkout_page_check" ' .  (isset($show_on_checkout) && $show_on_checkout == 'on' ? 'checked' : '').' />';
       echo '<span class="disabled_for_checkout" style="margin-left:30px;">'.$this->pro_image.'</span>';
    }
